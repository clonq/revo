...
module.exports = {
    ...
    start: function() {
        ...
        startWebServer().then(function(http){
            startComponents();
...
function startWebServer() {
    return new Promise(function(resolve, reject){
        ...
        http.listen(app.get('port'), function () {
            debug('web server v. ' + pkg.version + ' started on port '+app.get('port'));
            return resolve(http);
        });
    });
}
...
function startWebsocketServer(server) {
    ...
    if(server) {
        ...
        debug('piggyback websocket server started');
    } else {
            ...
            try {
                ...
                if((message.type === 'from-web') && message.event) {
                    ...
                    if(eventName === 'revo:client:ready') {
                        ...
                        // send the event back to the web layer
                        var msg = { type: 'revo-event', event: message.event };
                        if(message.payload) msg.payload = message.payload;
                        if(message.component) {
                            msg.component = message.component;
                            // add component config data to the payload
                            var unsafeComponentName = message.component.replace('_', '/');
                            var componentConfig = appConfig[unsafeComponentName];
                            delete componentConfig.name;
                            delete componentConfig.safename;
                            msg.config = componentConfig;
                        }
                        if(!!msg.config && (msg.config.type == 'web') && !!socketClient) {
                            delete msg.config.type;
                            socketClient.send(JSON.stringify(msg));
                        }
                    }
    ...
    try {
        ...
        for(componentName in components) {
            ...
            if(appConfig[componentOpts.name]) {
                ...
                componentOpts = _.defaults(configData, componentOpts);
            }
            if(componentOpts.type == 'common') {
                ...
                var opts = componentOpts;//components[componentName];
