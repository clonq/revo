...
function startWebBridge() {
    //todo: remove process.on('web:flow')?
...
// debug('web:flow', pin);
    ...
    // filter listening web components
    var wc = _.filter(webComponents, function(component){
        if (component.handles) return component;
    });
    // register event handlers
    if(wc.length) {
        socketClient.send(JSON.stringify({ type: 'revo-config', register: wc }));
    }
    // initialize web components in the client
    _.toArray(webComponents).forEach(function(component){
        socketClient.send(JSON.stringify({ type: 'revo-config', init: component }));
    });
}
...
function startWebsocketServer() {
    ...
    wss.on('connection', function connection(ws) {
        ...
        // setup web components in the client
        // var wc = _.filter(webComponents, function(component){
        //     // if (component.listen) return component;
        //     // if (component.handles) return component;
        //     return component;
        // });
        // if(wc.length) {
        //     var configMsg = { type: 'revo-config', register: wc };
        //     ws.send(JSON.stringify(configMsg));
        //     wc.forEach(function(component){
        //         var initMsg = { type: 'revo-config', init: component };
