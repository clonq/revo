...
//     },
    ...
    installComponent: function(path, name, opts) {
        ...
        return new Promise(function(resolve, reject){
            ...
            // if the source only contains an intermediate directory, make the the actual source 
            var subdirs = util.getSubdirs(source);
            var files = util.getFiles(source);
            if((files == 0) && (subdirs.length == 1)) source = [source, subdirs[0]].join('/');
