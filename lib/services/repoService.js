    ...
    web: {
        fetchTheme: function(theme){
            return new Promise(function (resolve) {
                //TODO: check local themes folder first
                debug('fetching theme from', theme.url);
                ...
                r.on('finish', function () {
                    ...
                    // if(theme.zippath) zip.extractEntryTo(theme.zippath+'/*', themeTargetDir, false, true);
                    zip.extractAllTo(themeTargetDir, true);//overwrite existing
                    var ret = (theme.zippath) ? [theme.name, theme.zippath].join('/') : theme.name;
                    resolve(ret);
                });
            });
        }
