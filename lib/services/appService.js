    ...
    .then(function(){
        ...
        .then(function(filedata){
            ...
            recipe.components.forEach(function(componentDefinition){
                var webComponent = component.parse(componentDefinition);
                webComponent.scripts().forEach(function(script){
                    var scriptSrc = ['components', webComponent.name, 'js', script.name].join('/');
