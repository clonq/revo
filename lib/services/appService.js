    ...
    deployApp: function(opts) {
        ...
        .then(function(recipe){
            ...
            if(method === 'scp') {
                ...
                //todo: delete temp zip when done
                if(recipe.verbose) console.log('copying', zipFilename, 'to', sshopts.host);
                util.scp(zipFilename, sshopts)
                .then(function(data){
                    // extract the app
                    var shortZipName = zipFilename.substring(zipFilename.lastIndexOf('/')+1);
                    if(recipe.verbose) console.log('extracting packaged app extracted to', sshopts.path);
                    sshopts.cmd = 'tar xvf '+shortZipName;
                    ...
                    .then(function(result){
                        ...
                        // remove remote zip
                        if(recipe.verbose) console.log('cleanup');
                        sshopts.cmd = 'rm '+shortZipName;
                        util.sshExec(sshopts)
                        .then(function(result){
                            if(recipe.verbose) {
                                console.log(result.stdOut);
...
function createConfig(recipe) {
    ...
    var filename = recipe.path + '/appconfig.json'
        ...
        if (recipe.components) {
            ...
            .then(function(){
                ...
                .then(function(){
                    ...
                    Promise.all(installs)
                    .then(function(){
console.log(fetches.length+' common component'+((fetches.length>1)?'s':'')+' installed')
...
function installDependencies(recipe) {
    // if((recipe.dependencies == 'on') || (recipe.dependencies == 0)) {
