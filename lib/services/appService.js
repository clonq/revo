...
function createPackage(recipe) {
    ...
    if(recipe.platform.type == 'web') pkg.dependencies['node-static'] = '0.7.6';
...
function installRevo(recipe) {
    ...
    return fs.writeFileAsync(startFilename, 'npm start')
	    .then(function(){
            ...
            // core components
            // util.installFile('components/revo/webbridgecomponent.js', recipe.path + '/container');
	    })
	    .then(function(){
	...
	return new Promise(function(resolve, reject){
        ...
        if (recipe.components) {
            ...
            .then(function(){
                ...
                debug('fetching '+fetches.length+' components')
                Promise.all(fetches)
                .then(function(){
                    debug(fetches.length+' components fetched')
                    return resolve(recipe);    
                })
                .catch(function(err){
